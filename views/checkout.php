...
if (isset($_POST['place_order'])) {
    ...
    $verify_cart = $conn->prepare("SELECT * FROM `panier` WHERE user_id = ?");
    ...
    if (isset($_GET['get_id'])) {
        ...
        if ($get_product->rowCount() > 0) {
            while ($fetch_p = $get_product->fetch(PDO::FETCH_ASSOC)) {
                $insert_order = $conn->prepare("INSERT INTO `commandes`(id, user_id, name, numero, email, adresse, adresse_type, methode, product_id, prix, quantite) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
                $insert_order->execute([create_unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $fetch_p['id'], $fetch_p['prix'], 1]);
        ...
        while ($f_cart = $verify_cart->fetch(PDO::FETCH_ASSOC)) {
            ...
            $insert_order = $conn->prepare("INSERT INTO `commandes`(id, user_id, name, numero, email, adresse, adresse_type, methode, product_id, prix, quantite) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
            $insert_order->execute([create_unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $f_cart['product_id'], $f_cart['prix'], $f_cart['quantite']]);
        ...
        $warning_msg[] = 'votre panier est vide!';
    }
                ...
                <div class="flex">
                    ...
                    <div class="box">
                        ...
                        <input type="text" name="street" required maxlength="50"
                            placeholder="e.g. street name & locality" class="input">
                        ...
                        <input type="number" name="pin_code" required maxlength="6" placeholder="e.g. 123456"
                            class="input" min="0" max="999999">
                    </div>
                </div>
